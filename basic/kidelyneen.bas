...
20 gosub 8300:gosub 8500:gosub 8200
30 gosub 2000
40 end

500 rem preinit
510 dim ac%(3),ad%(1,1):sd%=40:li%=3:sc=0
515 dim lc%(3,4,2),cv%(4)
520 print chr$(8);chr$(142);:i=rnd(0)
530 ad%(0,0)=1:ad%(0,1)=40:ad%(1,0)=39:ad%(1,1)=41
540 gosub 27000:return

1000 rem reinit
1005 poke 646,1:print chr$(147);:ac%=0
1006 vw%(0)=0:vw%(1)=0:vw%(2)=0:ti$="000000"
...
1930 jo%=0:poke 53287,13:n%=19:gosub 32000:gosub 1000
...
1980 n%=11:gosub 32000:run

1999 rem player movement, main game loop
2000 pm%=ps%:bl%=0:kf%=0:pr%=-1:pj%=0:oj%=0:fx%=0:poke 198,0
...
2035 goto 2002
2040 ed%=ed%-10:if ps%<pm% then pm%=ps%
...
5510 ot%=ft%:ft%=0:sa%=8192:se%=9192:ta%=1024
5540 ct=54272+ta%
...
5610 gosub 8300
5620 ot=(ft%-ot%):if ot>0 then ot=ot*ot:sc=sc+int(log(ot)*100)
5630 gosub 8500:return
...
8500 rem print score
8510 a$=str$(sc):a$=mid$(a$,2):b$=right$("000000"+a$,6)
8520 a$=left$(b$,3):sp%=10304:gosub 20000
8530 a$=right$(b$,3):sp%=10368:gosub 20000
8540 return
...
10320 ch%=32:fc%=bc%:sl%=9472:sh%=9472
10400 ad%=ta%+40*y%+x%
10410 gosub 10700
...
10950 rem copy - todo remove asm example again...
...
30000 rem death animation
30005 lp%=4:cv%(0)=1:cv%(1)=7:cv%(2)=5:cv%(3)=10:cv%(4)=2
30010 cv%=peek(ps%):cc%=peek(ps%+54272)
30020 for i=0 to 3:ac%(i)=ps%:for p=0 to 4
30025 lc%(i,p,0)=ps%:lc%(i,p,1)=cv%:lc%(i,p,2)=cc%:next:next
30026 gosub 39650
30030 for i=0 to 20:for p=0 to 3:p%=p
30035 az%=ac%(p%)
30050 cv%=lc%(p%,lp%,0):if cv%=0 then 30090
30060 x%=lc%(p%,lp%,1):y%=lc%(p%,lp%,2)
30070 if cv%>1023 then if cv%<2024 then poke cv%,x%:poke cv%+54272,y%
30090 lc%(p%,lp%,0)=az%
30100 cv%=peek(az%):lc%(p%,lp%,1)=cv%
30110 cv%=peek(az%+54272):lc%(p%,lp%,2)=cv%
30115 if az%>1023 then if az%<2024 then poke az%,81:poke az%+54272,cv%(lp%)
30120 lp%=lp%-1:if lp%=-1 then lp%=4
30130 next:gosub 8800:next
30140 for i=0 to 3:for p=0 to 4:cv%=lc%(i,p,0)
30150 poke cv%,lc%(i,p,1):poke cv%+54272,lc%(i,p,2)
30160 next:next:return
...
39700 rem start sound
39710 at%=9:dd%=8:el%=0:rl%=8:lq%=180:hq%=10
39720 wf%=16:pt%=25:im%=0:gosub 28000
39730 at%=9:dd%=8:el%=0:rl%=4:lq%=180:hq%=12
39740 wf%=16:pt%=20:im%=0:gosub 28000
39750 at%=9:dd%=8:el%=0:rl%=6:lq%=180:hq%=15
39760 wf%=16:pt%=30:im%=0:gosub 28000:return
